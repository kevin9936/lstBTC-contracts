...
/**
 * @title BtcUtils
 * @dev Bitcoin transaction parsing and utility library
 *
 ...
 * - Enhanced P2PK script validation with compressed public key support
 * - Added comprehensive Bitcoin script type detection
 * - Improved error handling and gas efficiency
 *
...
library BtcUtils {
    ...
    function outputScriptToAddress(bytes calldata outputScript, bool mainnet) public pure returns (bytes memory) {
        if (isP2PKOutput(outputScript)) {
            return parsePayToPubKey(outputScript);
        }
    ...
    /// @notice Check if a raw output script is a pay-to-public-key output
    /// @param pkScript the fragment of the raw transaction containing the raw output script
    /// @return Whether the script has a pay-to-public-key output structure or not
    function isP2PKOutput(bytes calldata pkScript) public pure returns (bool) {
        return pkScript.length == 3 + SHA256_SIZE &&
            uint8(pkScript[0]) == 0x21 &&
            (uint8(pkScript[1]) == 0x02 || uint8(pkScript[1]) == 0x03) &&
            pkScript[34] == OpCodes.OP_CHECKSIG;
    }
    ...
    function isP2PKHOutput(bytes calldata pkScript) public pure returns (bool) {
    ...
    function isP2SHOutput(bytes calldata pkScript) public pure returns (bool) {
    ...
    function isP2WPKHOutput(bytes calldata pkScript) public pure returns (bool) {
    ...
    function isP2WSHOutput(bytes calldata pkScript) public pure returns (bool) {
    ...
    function isP2TROutput(bytes calldata pkScript) public pure returns (bool) {
    ...
    /// @notice Check if a Bitcoin output script is a valid supported script type
    /// @param pkScript the fragment of the raw transaction containing the raw output script
    /// @return Whether the script is a valid supported script type
    function isValidPkScript(bytes calldata pkScript) public pure returns (bool) {
        return isP2PKOutput(pkScript) ||
               isP2PKHOutput(pkScript) ||
               isP2SHOutput(pkScript) ||
               isP2WPKHOutput(pkScript) ||
               isP2WSHOutput(pkScript) ||
               isP2TROutput(pkScript);
    }

    /// @notice Parse a raw pay-to-public-key output script to get the public key
    /// @param outputScript the fragment of the raw transaction containing the raw output script
    /// @return The public key bytes
    function parsePayToPubKey(bytes calldata outputScript) public pure returns (bytes memory) {
        require(isP2PKOutput(outputScript), "Script hasn't the required structure");
        return outputScript[1:34];
    }
