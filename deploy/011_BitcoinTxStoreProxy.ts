...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const bitcoinTxStoreLogic = await deployments.get("BitcoinTxStoreLogic");
  ...
  const args = [
    bitcoinTxStoreLogic.address,
    initCode
  ];

  const deployedContract = await deploy("BitcoinTxStoreProxy", {
    from: deployer,
    log: true,
  ...
  if (network.name != "hardhat" && process.env.ETHERSCAN_API_KEY && process.env.VERIFY_OPTION == "1") {
    await verify(
      ...
      args,
      "contracts/bitcoin-tx-store/BitcoinTxStoreProxy.sol:BitcoinTxStoreProxy",
      hre
    );
  }
};
