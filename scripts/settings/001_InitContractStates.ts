...
async function setLstBTCBridgeStates(
  ...
  const oldBitcoinTxStore = await bridgeInst.bitcoinTxStore();
  if (oldBitcoinTxStore != bitcoinTxStoreInst.address) {
    const tx = await bridgeInst.setBitcoinTxStore(bitcoinTxStoreInst.address);
    await tx.wait(1);
    logger.color('green').log(`✅ LstBTCBridge.bitcoinTxStore = ${bitcoinTxStoreInst.address} (tx: ${tx.hash})`);
  } else {
    logger.color('yellow').log(`⏭️  LstBTCBridge.bitcoinTxStore already set to ${oldBitcoinTxStore}`);
  }
...
async function setBitcoinTxStoreStates(bitcoinTxStoreInst: any, bridgeInst: any) {
  logger.color('blue').log("=========================================");
  logger.color('blue').bold().log("🔧 Setting BitcoinTxStore States...");
  logger.color('blue').log("=========================================");
  if (!bitcoinTxStoreInst) return;

  const ROLE_RELAYER = ethers.utils.keccak256(ethers.utils.toUtf8Bytes("ROLE_RELAYER"));
  if (!await bitcoinTxStoreInst.hasRole(ROLE_RELAYER, bridgeInst.address)) {
    const tx = await bitcoinTxStoreInst.grantRole(ROLE_RELAYER, bridgeInst.address);
    await tx.wait(1);
    assert(await bitcoinTxStoreInst.hasRole(ROLE_RELAYER, bridgeInst.address))
    logger.color('green').log(`✅ BitcoinTxStore.ROLE_RELAYER granted to ${bridgeInst.address} (tx: ${tx.hash})`);
  } else {
    logger.color('yellow').log(`⏭️  BitcoinTxStore.ROLE_RELAYER already granted to ${bridgeInst.address}`);
  }
}
...
async function setConfigRegistryStates(configRegistryInst: any) {
...

  {
    ...
    const result = await configRegistryInst.getPegInDepositDustAmount(Math.floor(Date.now() / 1000 + floatTime))
...

  {
    ...
    const result = await configRegistryInst.getPegInTreasuryFeeRate(Math.floor(Date.now() / 1000 + floatTime))
...

  {
    ...
    const result = await configRegistryInst.getPegInTreasuryFeeShares(Math.floor(Date.now() / 1000 + floatTime))
...

  {
    ...
    const result = await configRegistryInst.getPegOutRedeemDustAmount(Math.floor(Date.now() / 1000 + floatTime))
...

  {
    ...
    const result = await configRegistryInst.getPegOutTransactionFee(Math.floor(Date.now() / 1000 + floatTime))
...

  {
    ...
    const result = await configRegistryInst.getPegOutTreasuryFeeRate(Math.floor(Date.now() / 1000 + floatTime))
    logger.color('blue').log(`📊 Verified: treasury fee rate = ${result.toString()}`);
  }

  {
    ...
    const result = await configRegistryInst.getPegOutTreasuryFeeShares(Math.floor(Date.now() / 1000 + floatTime));
    logger.color('blue').log(`📊 Verified: recipients = ${result[0]}`);
    logger.color('blue').log(`📊 Verified: shares = ${result[1]}`);
  }
}
...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const bitcoinTxStoreInst = await getContractInst(deployments, "BitcoinTxStoreProxy", "BitcoinTxStoreLogic", "");
  ...
  // Set BitcoinTxStore states
  await setBitcoinTxStoreStates(bitcoinTxStoreInst, bridgeInst);
